...
class PerquisiteCalculator:
    ...
    def calculate_rent_free_accommodation(
        ...
        population_tier: str = ">40L",  # ">40L", "15L-40L", "<=15L"
        ...
        if is_government_employee:
            ...
            if regime == "2026":
                ...
                rate = 0.10 if population_tier == ">40L" else 0.07
                ...
                # 1962 Rules (Post-Notification 65/2023 amended rates)
                if is_owned_by_employer:
                    tier_rates = {">40L": 0.10, "15L-40L": 0.075, "<=15L": 0.05}
                    rate = tier_rates.get(population_tier, 0.10)
                    unfurnished_val = salary_for_period * rate
                    breakdown.append(f"Employer-owned in population tier '{population_tier}': {rate*100:.1f}% of salary (Rs. {salary_for_period:,.2f}) = Rs. {unfurnished_val:,.2f}")
                else:
                    ten_pct_salary = salary_for_period * 0.10
                    unfurnished_val = min(actual_lease_rent_paid, ten_pct_salary)
                    breakdown.append(f"Leased by employer: Min(Actual Rent Rs. {actual_lease_rent_paid:,.2f}, 10% Salary Rs. {ten_pct_salary:,.2f}) = Rs. {unfurnished_val:,.2f}")
    ...
    def calculate_motor_car(
        ...
        if usage_type == "personal":
            ...
            return {
                ...
                "computation_steps": [
                    f"100% Personal Use: Actual Running Expenses (Rs. {actual_running_expense_incurred:,.2f}) + 10% Normal Wear & Tear (Rs. {annual_depreciation:,.2f})",
                    f"Prorated for {months_used} months = Rs. {gross_val:,.2f}",
                    f"Less recovered Rs. {amount_recovered:,.2f} = Rs. {taxable_val:,.2f}"
                ]
            }
        ...
        if is_employer_owned_or_hired and is_employer_expenses_met:
            ...
            total_reimbursed = actual_running_expense_incurred * (months_used / 12)
            allowable_exemption = exemption_rate * months_used
            driver_exemption = (900.0 * months_used) if is_driver_provided else 0.0
            taxable_val = max(0.0, total_reimbursed - (allowable_exemption + driver_exemption) - amount_recovered)
            return {
                "perquisite_type": "motor_car",
                "statutory_rule": "Rule 3(2)",
                "regime": regime,
                "usage_type": "mixed_employee_owned",
                "reimbursement_amount": round(total_reimbursed, 2),
                "statutory_deduction": round(allowable_exemption + driver_exemption, 2),
                "taxable_perquisite_value": round(taxable_val, 2),
                "computation_steps": [
                    f"Employee-owned car: Actual employer reimbursement Rs. {total_reimbursed:,.2f}",
                    f"Less statutory exemption ({months_used} mos @ Rs. {exemption_rate:,.2f}): Rs. {allowable_exemption:,.2f}",
    ...
    def calculate_interest_free_loan(
        ...
        if is_aggregate_loans_under_20k or max_outstanding_monthly_balance <= 20000.0:
            return {
                "perquisite_type": "interest_free_loan",
                "statutory_rule": "Rule 3(7)(i)",
                "regime": regime,
                "is_exempt": True,
                "exemption_reason": "Aggregate loan amount does not exceed statutory threshold of Rs. 20,000.",
                "taxable_perquisite_value": 0.0,
                "computation_steps": [
                    f"Loan amount Rs. {max_outstanding_monthly_balance:,.2f} <= statutory Rs. 20,000 threshold.",
                    "Exempt under Rule 3(7)(i) proviso 1.",
                    "Taxable Perquisite = Rs. 0.00"
                ]
            }

        if is_medical_treatment_specified_disease and medical_insurance_reimbursement == 0.0:
            return {
                "perquisite_type": "interest_free_loan",
                "statutory_rule": "Rule 3(7)(i)",
                "regime": regime,
                "is_exempt": True,
                "exemption_reason": "Loan given for medical treatment of specified diseases under Rule 3A.",
                "taxable_perquisite_value": 0.0,
                "computation_steps": [
                    "Loan granted for medical treatment of specified diseases under Rule 3A.",
                    "Exempt under Rule 3(7)(i) proviso 2.",
                    "Taxable Perquisite = Rs. 0.00"
                ]
            }

        rate_diff = max(0.0, sbi_benchmark_rate_pct - actual_interest_rate_charged_pct)
        breakdown.append(f"SBI Benchmark Lending Rate (1st day of FY): {sbi_benchmark_rate_pct:.2f}%")
